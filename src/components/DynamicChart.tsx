    ...
    <Card className="w-full h-[450px] border-border/50 shadow-sm rounded-xl">
      ...
      <CardContent className="h-[320px] flex items-center justify-center">
        {loading ? (
          ...
          <p className="text-muted-foreground font-medium">No dynamic data found yet!</p>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="hsl(var(--border))" />
              <XAxis dataKey="name" tick={{ fill: "hsl(var(--foreground))", fontSize: 12 }} tickLine={false} axisLine={false} />
              <YAxis tick={{ fill: "hsl(var(--foreground))", fontSize: 12 }} tickLine={false} axisLine={false} />
              <Tooltip 
